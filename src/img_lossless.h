...
static size_t lzma_decompress_img(const uint8_t* in, size_t in_len, uint8_t* out, size_t out_cap) {
    ...
    SizeT destLen = (SizeT)out_cap; ISzAlloc alloc = { SzAllocFn, SzFreeFn }; ELzmaStatus st = 0;
...
size_t img_reencode_lossless(const uint8_t* in, size_t in_len,
    ...
    if (t == IMG_JPEG) {
        ...
        *(uint32_t*)(out + 13) = (uint32_t)scan_len; *(uint32_t*)(out + 17) = 0; // zcmp_len placeholder
        ...
        size_t zcap = out_cap - wr; size_t zlen = deflate_compress(in + scan_off, scan_len, out + wr, zcap, 9);
        if (zlen == 0) return 0; *(uint32_t*)(out + 17) = (uint32_t)zlen; return wr + zlen;
    }
